...
PALETTE = {
    ...
    "muted": "#6b6b6b",
    "accent": "#2b2b2b",
    "border": "#a0a0a0",     # grey border for canvas
    "grid": "#9e9e9e",
}
...
class JapaneseCharacterRecognizer:
    def __init__(self, root):
        ...
        try:
            ...
            self.kanji_model = load_model("kanji01.h5")
        ...
        # Language mode
        self.current_language = "en"  # "en" or "jp"
    ...
    def _build_ui(self):
        ...
        self.brush_scale = ttk.Scale(
            brush_panel, from_=2, to=28, value=self.line_width, command=self._update_brush
        )
        ...
        # Language dropdown under buttons
    ...
    def _preprocess_for_model(self):
        ...
        mask = arr < 255
        if not mask.any():
            base = Image.new("L", (self.model_input, self.model_input), 0)
            return np.array(base, dtype=np.float32).reshape(1, self.model_input, self.model_input, 1) / 255.0
        ys, xs = np.where(mask)
        x1, x2 = xs.min(), xs.max()
        y1, y2 = ys.min(), ys.max()
        ...
        resized = square.resize((self.model_input, self.model_input), Image.LANCZOS)
        inverted = ImageOps.invert(resized)
        x = np.array(inverted, dtype=np.float32) / 255.0
        return x.reshape(1, self.model_input, self.model_input, 1)
