...
class JapaneseCharacterRecognizer:
    ...
    def _build_ui(self):
        ...
        self.brush_scale = ttk.Scale(brush_panel, from_=2, to=28, value=self.line_width, command=self._update_brush)
        ...
        self.kutsu_rb = ttk.Radiobutton(self.mode_box, text="Kuzushiji", variable=self.mode_var, value="kutsushiji", command=self.change_mode)
    ...
    def _preprocess_for_model(self):
        ...
        mask = arr < 255
        if not mask.any():
            base = Image.new("L", (self.model_input, self.model_input), 255)
