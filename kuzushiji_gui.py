...
from kuzujapanese import label 

class KuzushijiRecognizer:
    def __init__(self, root):
        self.root = root
        self.root.title("Japanese Kuzushiji Recognizer")
        
        # Load the trained model
        try:
            self.model = load_model("kuzushiji.h5")
    ...
    def setup_ui(self):
        ...
        # Labels
        self.instruction = Label(self.root, text="Draw a kuzushiji character", 
                               font=("Helvetica", 12))
    ...
    def clear(self):
        ...
        self.instruction.config(text="Draw a kuzushiji character")
    
    def predict(self):
        try:
            # Preprocess image
            img = self.drawing.resize((40, 40))
            ...
            if confidence < 0.85:  # 85% confidence threshold
                self.result.config(text="Not recognized as kuzushiji", fg="red")
            else:
